  ...
  return <aside className="fresh-assumptions" aria-label="Projection assumptions">
    ...
    {(activeTab === "overview" || activeTab === "cash") && <>
      <section className="fresh-input-group"><h3>Base salary</h3>
        ...
        <NumericDraftInput label="Annual growth (%)" type="number" step="0.1" value={String(scenario.salary.annualGrowth * 100)} parse={(raw) => { const value = finiteNumber(-99.999)(raw); return value === undefined ? undefined : value / 100; }} onCommit={(annualGrowth) => setSalary({ annualGrowth })} />
      </section>
      ...
      <section className="fresh-input-group"><div className="fresh-group-heading"><h3>Annual bonuses</h3><button type="button" onClick={addBonus}>Add bonus</button></div>
        {scenario.bonuses.map((bonus, index) => <fieldset className="fresh-array-item" key={bonus.id}><legend>{`Bonus ${index + 1}`}</legend>
          ...
          {bonus.mode === "percent" ? <NumericDraftInput label="Target bonus (%)" type="number" min="0" step="0.1" value={String(bonus.amount * 100)} parse={(raw) => { const value = finiteNumber(0)(raw); return value === undefined ? undefined : value / 100; }} onCommit={(amount) => updateBonus(index, { ...bonus, amount })} /> : <NumericDraftInput label={`Fixed bonus (${bonus.currency})`} type="number" min="0" step="0.01" value={minorToDecimalDraft(bonus.amountMinor)} parse={minorDraft} onCommit={(amountMinor) => updateBonus(index, { ...bonus, amountMinor })} />}
          <NumericDraftInput label="Achievement (%)" type="number" min="0" max="200" value={String(bonus.achievement * 100)} parse={(raw) => { const value = finiteNumber(0, 200)(raw); return value === undefined ? undefined : value / 100; }} onCommit={(achievement) => updateBonus(index, { ...bonus, achievement })} />
      ...
      <section className="fresh-input-group"><div className="fresh-group-heading"><h3>Equity assets</h3><button type="button" onClick={addAsset}>Add asset</button></div>
        {scenario.equityAssets.map((asset, index) => <fieldset className="fresh-array-item" key={asset.id}><legend>{asset.name}</legend>
          ...
          <CurrencyInput label="Asset currency" value={asset.currency} onChange={(currency) => updateAsset(index, { currency })} />
           ...
           <NumericDraftInput label="Annual valuation growth (%)" type="number" step="0.1" value={String(asset.annualGrowth * 100)} parse={(raw) => { const value = finiteNumber(-99.999)(raw); return value === undefined ? undefined : value / 100; }} onCommit={(annualGrowth) => updateAsset(index, { annualGrowth })} />
    ...
    {activeTab === "taxfx" && <>
      <section className="fresh-input-group"><h3>Tax</h3><label>Tax mode<select value={scenario.tax.mode} onChange={(event) => setScenario({ ...scenario, tax: { ...scenario.tax, mode: event.target.value as Scenario["tax"]["mode"] } })}><option value="blended">Blended effective rate</option><option value="component">By component</option></select></label>{scenario.tax.mode === "blended" ? <NumericDraftInput label="Effective rate (%)" type="number" min="0" max="100" value={String(scenario.tax.blendedRate * 100)} parse={(raw) => { const value = finiteNumber(0, 100)(raw); return value === undefined ? undefined : value / 100; }} onCommit={(blendedRate) => setScenario({ ...scenario, tax: { ...scenario.tax, blendedRate } })} /> : <><p className="input-note">Component rates are local overrides. Resetting one uses the global blended rate without changing the others.</p>{COMPONENTS.map((component) => <div className="fresh-array-item" key={component}><NumericDraftInput label={`${componentLabel[component]} rate (%)`} type="number" min="0" max="100" value={String(scenario.tax.byComponent[component] * 100)} parse={(raw) => { const value = finiteNumber(0, 100)(raw); return value === undefined ? undefined : value / 100; }} onCommit={(rate) => setScenario({ ...scenario, tax: { ...scenario.tax, byComponent: { ...scenario.tax.byComponent, [component]: rate } } })} /><button type="button" onClick={() => setScenario({ ...scenario, tax: { ...scenario.tax, byComponent: { ...scenario.tax.byComponent, [component]: scenario.tax.blendedRate } } })}>Reset {componentLabel[component]} rate to global</button></div>)}</>}</section>
    ...
    {activeTab === "risk" && <section className="fresh-input-group"><h3>Simulation</h3>
      ...
      {riskFactorIds.map((factor) => {
        ...
        return <NumericDraftInput key={factor} label={`Volatility ${factor} (%)`} type="number" min="0" max="500" step="0.1" value={String(volatility * 100)} parse={(raw) => { const value = finiteNumber(0, 500)(raw); return value === undefined ? undefined : value / 100; }} onCommit={(value) => setScenario({ ...scenario, risk: { ...scenario.risk, volatilities: { ...scenario.risk.volatilities, [factor]: value } } })} />;
      })}
